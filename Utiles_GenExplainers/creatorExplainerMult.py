# -*- coding: utf-8 -*-
"""
Created on Thu Feb 24 10:12:49 2022
...
def menuTipoModeloExplainerMult():
    ...
    while tipo_modelo_mult == "":
        ...
        if opcion == 1:
            tipo_modelo_mult = "XGB"
        elif opcion == 2:
            tipo_modelo_mult = "DT"
        elif opcion == 3:
            tipo_modelo_mult = "RF"
        elif opcion == 4:
            tipo_modelo_mult = "KNN"
        elif opcion == 5:
            tipo_modelo_mult = "LogR"
        elif opcion == 6:
            tipo_modelo_mult = "MLP"         
        elif opcion == 7:
            tipo_modelo_mult = "Exit"
        else:
            print ("Introduce un numero entre 1 y 7")
    return tipo_modelo_mult
...
if __name__ == "__main__":
    ...
    tipo_modelo_mult = menuTipoModeloExplainerMult()
    if tipo_modelo_mult == "Exit":
        sys.exit()
    ...
    print("Calculando...")
    if tipo_modelo_mult == "XGB":
        explainer = shap.TreeExplainer(model)
    elif tipo_modelo_mult == "DT":
        explainer = shap.TreeExplainer(model)
    elif tipo_modelo_mult == "RF":
        explainer = shap.TreeExplainer(model)
    elif tipo_modelo_mult == "KNN":
    ...
    elif tipo_modelo_mult == "LogR":
        print("Seleccionar dataset con valores X del entrenamiento (necesario para generación del explainer)")
        path_datos_entrenamiento = loadFile()
        X_train = pd.read_csv(path_datos_entrenamiento) # ,sep=';')
        explainer = shap.Explainer(model, X_train)
    elif tipo_modelo_mult == "MLP":
        print("Seleccionar dataset con valores X del entrenamiento (necesario para generación del explainer)")
        path_datos_entrenamiento = loadFile()
        X_train = pd.read_csv(path_datos_entrenamiento) # ,sep=';')
